...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        for(int i=0;i<inorder.size();i++){  //put inorder array in hashmap
            mp[inorder[i]] = i;
        }
    ...
    TreeNode* buildTree(vector<int>& preorder, int preStart, int preEnd,
        ...
        int inRoot = mp[root->val];  //finding root in inorder
        int numsleft = inRoot - inStart; //numbers on left in inorder
